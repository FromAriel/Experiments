...
const BS_BOID_FISH_SCRIPT_SH := preload("res://scripts/boids/boid_fish.gd")
...
func _BS_ensure_fish_scene_exists_IN() -> void:
    ...
    var root := BS_BOID_FISH_SCRIPT_SH.new() as Node2D
...
func _BS_spawn_fish_IN(arch: FishArchetype) -> void:
    ...
    if BS_fish_scene_IN != null and is_instance_valid(BS_fish_scene_IN):
        ...
        fish = BS_BOID_FISH_SCRIPT_SH.new()
    # Position & depth
    if BS_environment_IN != null:
...
func _physics_process(delta: float) -> void:
    ...
    for fish in BS_fish_nodes_SH:
        ...
        _BS_apply_wall_slide_IN(fish, delta)
...
func _BS_update_fish_IN(fish: BoidFish, delta: float) -> void:
    ...
    if use_count > 0:
        # alignment
        var ali_vec = (use_ali / use_count).normalized() * BS_config_IN.BC_max_speed_IN
        ali_vec -= fish.BF_velocity_UP
...
func _BS_apply_wall_slide_IN(fish: BoidFish, delta: float) -> void:
    if BS_environment_IN == null:
        return
    var b = BS_environment_IN.TE_boundaries_SH
    var eff_min_x = b.position.x + BS_hard_margin_IN
    ...
    if fish.position.x <= eff_min_x and fish.BF_velocity_UP.x < 0.0:
        fish.BF_velocity_UP.x = lerp(fish.BF_velocity_UP.x, 0.0, delta * 5.0)
    elif fish.position.x >= eff_max_x and fish.BF_velocity_UP.x > 0.0:
        fish.BF_velocity_UP.x = lerp(fish.BF_velocity_UP.x, 0.0, delta * 5.0)

    if fish.position.y <= eff_min_y and fish.BF_velocity_UP.y < 0.0:
        fish.BF_velocity_UP.y = lerp(fish.BF_velocity_UP.y, 0.0, delta * 5.0)
    elif fish.position.y >= eff_max_y and fish.BF_velocity_UP.y > 0.0:
        fish.BF_velocity_UP.y = lerp(fish.BF_velocity_UP.y, 0.0, delta * 5.0)
