...
# gdlint:ignore = duplicated-load
    ...
    if BS_environment_IN != null:
        ...
        fish.BF_position_UP = Vector3(
            center.x, center.y, BS_rng_UP.randf_range(0.0, BS_environment_IN.TE_size_IN.z)
        )
        fish.position = Vector2(fish.BF_position_UP.x, fish.BF_position_UP.y)
        fish.BF_environment_IN = BS_environment_IN
    else:
        fish.BF_position_UP = Vector3(
            BS_rng_UP.randf_range(-50, 50), BS_rng_UP.randf_range(-30, 30), 0.0
        )
        fish.position = Vector2(fish.BF_position_UP.x, fish.BF_position_UP.y)
...
func _BS_update_fish_IN(fish: BoidFish, delta: float) -> void:
    ...
    if use_count > 0:
        ...
        # cohesion
        var coh_vec: Vector3 = (use_coh / use_count) - fish.BF_position_UP
        ...
        # separation
        var sep_vec: Vector3 = use_sep / use_count
    ...
    if BS_environment_IN != null:
        ...
        if fish.BF_position_UP.z < b.position.z + BS_hard_margin_IN + BS_boundary_margin_IN:
            var dz = (
                (b.position.z + BS_hard_margin_IN + BS_boundary_margin_IN - fish.BF_position_UP.z)
                / BS_boundary_margin_IN
            )
            push.z += dz
            wall_factor = max(wall_factor, dz)
        elif (
            fish.BF_position_UP.z
            > b.position.z + b.size.z - BS_hard_margin_IN - BS_boundary_margin_IN
        ):
            var dz = (
                (
                    fish.BF_position_UP.z
        ...
        if push != Vector3.ZERO:
            ...
            BS_steer_UP += (
                (center3 - fish.BF_position_UP).normalized() * BS_wall_nudge_IN * wall_factor
            )
    ...
    if BS_environment_IN != null:
        ...
        var eff_min_z2 = b2.position.z + BS_hard_margin_IN
        var eff_max_z2 = b2.position.z + b2.size.z - BS_hard_margin_IN
        ...
        if fish.BF_position_UP.z < eff_min_z2:
            ...
            if fish.BF_velocity_UP.z < 0:
                fish.BF_velocity_UP.z = min(fish.BF_velocity_UP.z + BS_hard_decel_IN * delta, 0)
        elif fish.BF_position_UP.z > eff_max_z2:
            fish.BF_position_UP.z = eff_max_z2
            if fish.BF_velocity_UP.z > 0:
                fish.BF_velocity_UP.z = max(fish.BF_velocity_UP.z - BS_hard_decel_IN * delta, 0)
    ...
    if BS_environment_IN != null:
        ...
        fish.BF_position_UP.z = lerp(
            fish.BF_position_UP.z, fish.BF_target_depth_SH, fish.BF_depth_lerp_speed_IN * delta
        )
        fish.BF_depth_UP = fish.BF_position_UP.z
        var ratio := fish.BF_position_UP.z / BS_environment_IN.TE_size_IN.z
        fish.modulate = Color(
            1.0 - ratio * 0.5, 1.0 - ratio * 0.5, 1.0 - ratio * 0.5, lerp(1.0, 0.4, ratio)
        )
