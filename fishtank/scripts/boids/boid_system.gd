...
@export var BS_bounce_damp_IN: float = 0.6

@export var BS_grid_cell_size_IN: float = 100.0

# Tint colors for each group (red, green, blue, yellow, magenta, cyan)
var BS_group_colors := [
    Color(1, 0, 0),
    Color(0, 1, 0),
    Color(0, 0, 1),
...
func _BS_spawn_fish_IN(arch: FishArchetype) -> void:
    ...
    if BS_fish_scene_IN != null and is_instance_valid(BS_fish_scene_IN):
        ...
        fish = BS_BOID_FISH_SCRIPT_UP.new()
    # Position & depth
    if BS_environment_IN != null:
...
func _BS_update_fish_IN(fish: BoidFish, delta: float) -> void:
    ...
    # soft‐wall repulsion with gradual damping
    var damp_x = 0.0
    var damp_y = 0.0
    if BS_environment_IN != null:
        ...
        if fish.position.x < soft_min_x:
            damp_x = (soft_min_x - fish.position.x) / BS_boundary_margin_IN
            steer.x += BS_boundary_force_IN * damp_x
        elif fish.position.x > soft_max_x:
            damp_x = (fish.position.x - soft_max_x) / BS_boundary_margin_IN
            steer.x -= BS_boundary_force_IN * damp_x

        if fish.position.y < soft_min_y:
            damp_y = (soft_min_y - fish.position.y) / BS_boundary_margin_IN
            steer.y += BS_boundary_force_IN * damp_y
        elif fish.position.y > soft_max_y:
            damp_y = (fish.position.y - soft_max_y) / BS_boundary_margin_IN
            steer.y -= BS_boundary_force_IN * damp_y

    # apply movement with damping near walls
    var velocity = (fish.BF_velocity_UP + steer * delta).limit_length(BS_config_IN.BC_max_speed_IN)
    if damp_x > 0.0:
        velocity.x = lerp(velocity.x, 0.0, clamp(damp_x, 0.0, 1.0))
    if damp_y > 0.0:
        velocity.y = lerp(velocity.y, 0.0, clamp(damp_y, 0.0, 1.0))
    ...
    if BS_environment_IN != null:
        ...
        if fish.position.x < eff_min_x2:
            ...
            if fish.BF_velocity_UP.x < 0:
                fish.BF_velocity_UP.x = abs(fish.BF_velocity_UP.x) * BS_bounce_damp_IN
        ...
        if fish.position.y < eff_min_y2:
            ...
            if fish.BF_velocity_UP.y < 0:
                fish.BF_velocity_UP.y = abs(fish.BF_velocity_UP.y) * BS_bounce_damp_IN
