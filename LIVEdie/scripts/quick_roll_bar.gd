...
@export var qrb_roll_button_font_size: int = 28
...
func _update_queue_display() -> void:
    ...
    for entry in qrb_queue:
        ...
        chip.add_theme_font_size_override("font_size", qrb_chip_font_size)
...
func _build_custom_panel() -> void:
    ...
    for key in order:
        ...
        btn.add_theme_font_size_override("font_size", qrb_button_font_size)
        if key == "DEL":
            btn.text = "\u232b"
        elif key == "OK":
            btn.text = "\u2714"
...
func _qrb_apply_scale() -> void:
    ...
    var roll_font: int = int(qrb_roll_button_font_size * scale)
